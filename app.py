...
st.subheader("Acompanhe as cotações em tempo real e acesse os anúncios diretamente.")
...
# --- ABA 2: MERCADO AOA (ANGOLA com Links Diretos) ---
with aba_aoa:
    st.info("💡 Nota para Angola: Clique no botão verde '🔗 Abrir Anúncio na Binance' para conferir os termos ocultos e conversar com o anunciante no chat oficial.")
    ...
    with col1_aoa:
        ...
        if dados_aoa_buy:
            for i, anuncio in enumerate(dados_aoa_buy, 1):
                ...
                # Pegando o ID do anúncio para gerar o link
                id_anuncio = adv['advNo']
                link_binance = f"https://p2p.binance.com/pt-PT/advertisement?adsNo={id_anuncio}"
                ...
                with st.container(border=True):
                    ...
                    # Botão que redireciona para a Binance em uma nova aba
                    st.link_button("🔗 Abrir Anúncio na Binance", link_binance, type="secondary")
...
st.markdown("---")
st.caption("Dados obtidos diretamente da API pública da Binance.")